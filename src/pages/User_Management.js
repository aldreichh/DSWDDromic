...
import UM_Contents from '../components/UM_Contents';
...
export default function Data_Visualization() {
    ...
    useEffect(() => {
        const fetchData = async () => {
          try {
            const provinceResponse = await axios.get('http://127.0.0.1:8000/api/provincesindex');
            const usersResponse = await axios.get('http://127.0.0.1:8000/api/showallusers');
            const citiesResponse = await axios.get('http://127.0.0.1:8000/api/citiesindex');
            const barangayResponse = await axios.get('http://127.0.0.1:8000/api/barangaysindex');
            const disasterResponse = await axios.get('http://127.0.0.1:8000/api/getindexdisaster');
    
            setallProvince(provinceResponse.data);
            setUsers(usersResponse.data.data);
            setcitiesIndex(citiesResponse.data);
            setbarangayIndex(barangayResponse.data);
            setdisastersIndex(disasterResponse.data);
          } catch (error) {
    ...
    <>
        ...
        <ThemeProvider theme={defaultTheme}>
            <Box sx={{ display: 'flex' }}>
                ...
                <Drawer variant="permanent" open={open}>
                    ...
                    {open ? (
                        ...
                        <ListItem sx={{ backgroundColor: '#0284c7',
                                    ...
                                    }} button divider onClick={userManagementHandler}> 
                            <ListItemIcon sx={{ color: 'white' }}>
                                <ManageAccountsIcon />
                            </ListItemIcon>
                            <ListItemText sx={{ color: 'white' }} primary="User Management" />
                        </ListItem>
                        )} 
                ...
                >
                    <Toolbar />
                    <UM_Contents    provinceIndex={provinceIndex} citiesIndex={citiesIndex} 
                                    barangayIndex={barangayIndex} disasters={disasters} users={users}/>
                </Box>
            </Box>
        </ThemeProvider>
    </>
